  ...
  const mode = await inquirer.prompt([
    {
      name: "modeSelect",
      type: "list",
      message: chalk.cyanBright.bold.italic(
        "Please select mode you want to play"
      ),
      choices: [
        chalk.green("Easy"),
        chalk.yellow("Medium"),
...
async function computerGuess(temp_var: number) {
  const randomNumber = Math.floor(Math.random() * temp_var + 1);
  return randomNumber;
  console.log(randomNumber);
}
async function truns(turn: number, n: number) {
...
async function gameOver(i: number) {
  if (i == turn) {
